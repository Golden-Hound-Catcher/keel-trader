...
class TestApiAfterPaperCycle(unittest.TestCase):
    ...
    def test_status_and_config_expose_kill_switch(self):
        ...
        self.assertIn("shadow_near_probe_edge_mode", r.json())
        self.assertEqual(r.json()["shadow_near_probe_edge_mode"], "round_trip")
        self.assertIn("shadow_near_probe_hurdle_bps", r.json())
        ...
        self.assertIn("first_live", r.json())
        fl = r.json()["first_live"]
        self.assertIsInstance(fl, dict)
        self.assertIn("allowed_now", fl)
        self.assertFalse(fl["allowed_now"])
        self.assertIn("human_steps", fl)
        self.assertIsInstance(fl["human_steps"], list)
        self.assertGreaterEqual(len(fl["human_steps"]), 5)
        self.assertIn("suggested_live_caps", fl)
        self.assertIn("blockers", fl)
        self.assertIn("kill_switch", fl)
        self.assertIn("shadow_mode", fl)
        self.assertIn("capability", fl)
