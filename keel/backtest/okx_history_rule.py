...
P0 adds optional Supertrend / ATR trailing-stop markout on the same fires
(no distant TP; ratchet SL each closed bar; flip or timeout). Measurement
only — does not change live exits.
...
# P0 Supertrend trail (UT Bot / ta.supertrend geometry). Timeout is longer
# than one 15m bar so the ratchet can actually move — F2c's 900s timeout
# made TP/trail indistinguishable from "exit next bar".
DEFAULT_TRAIL_TIMEOUT_SECONDS = 14_400  # 4h = 16×15m
DEFAULT_TRAIL_SL_ATR = 1.0
# Exit-path research (measurement only): MFE/MAE in R (1R = 1.0 ATR SL).
    ...
    *,
    ...
    if include_trail:
        ...
        trail_summary = {
            ...
            "sl_atr": float(trail_sl_atr),
            "timeout_seconds": float(trail_timeout_seconds),
            "period": int(trail_period),
            "multiplier": float(trail_multiplier),
            "vs_fixed_horizon_seconds": int(clear_horizon_seconds),
        }
