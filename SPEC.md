...
## 15b. Phase E0/E1/E2A/E2B/E3/E3.1/F1 — full-gate measurement + trend-follow + fire cooldown + 4h filter + cohort isolate (gen-2)
...
**F2c (strategy compare):** Offline side-by-side on same OKX candles: (A) TF E3.1; (B) mean_revert; (C) TF ATR barrier 2.2/1.0; **(C2) maker 4bps RT reprice**; (D) Supertrend trail; (E) P1 regime; (F) P2 score; **(G) early TP 1.0 / 1R scale-half** + **MFE/MAE** path; **(H) squeeze_release + 4h time-stop**; **(H2) H maker reprice**. CLI `scripts/okx_history_strategy_compare.py`. Still E0 freeze — no live exit flip. See RUNBOOK.
...
| Date | Note |
|------|------|
| 2026-09-09 | **P5 squeeze-release + maker reprice (measurement):** `KEEL_RULE_VARIANT=squeeze_release` — first TTM expansion bar + Supertrend + 1h; 4h hard time-stop barrier (not trail). Same-candle H: FG=6, net **−22bps** (sparse, stop). C2 maker reprice of TF barrier: **−4.6bps** vs C **−10.6bps** (fee +6bps, still ≤0; 100% fill assumed). Live default still mean_revert/rule; E0 freeze |
| 2026-09-09 | **MFE/MAE + early TP (measurement):** `excursion_markout` (1R=1.0 ATR; 15m + 4h/SL path); G legs = TP 1.0 full close and 1R scale-half. 15m rarely reaches 1R; early TP **worsens net** vs 2.2 barrier. Live exits unchanged; E0 freeze |
| 2026-09-09 | **P2 scored router + P4 LLM veto (measurement):** `KEEL_RULE_VARIANT=score` — same P1 regimes; trend 0–5 fire≥4 (must HTF+Supertrend; RSI chase veto); range must extreme %B+VWAP. Compare CLI leg F. `KEEL_DECISION_POLICY=llm_veto` overlay: LLM cannot upgrade WAIT or flip side. Live default still mean_revert/rule; E0 freeze |
| 2026-09-09 | **P1 regime router (measurement):** `KEEL_RULE_VARIANT=regime` — squeeze/shock WAIT; trend=TF-like 15m+1h (4h default off); range=percent_b+VWAP+RSI fade. Factors: Keltner/squeeze/`classify_market_regime`. Compare CLI leg E. Live default still mean_revert; E0 freeze |
| 2026-09-09 | **P0 Supertrend trail (measurement):** `calculate_supertrend` factor + `trailing_supertrend_exit_markout`; compare CLI leg D on same TF fires (no TP; ATR trail; 4h timeout). Live exits unchanged; E0 freeze |
