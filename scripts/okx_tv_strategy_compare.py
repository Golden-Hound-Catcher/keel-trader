#!/usr/bin/env python3
"""
F5: compare trend_follow vs supertrend vs donchian (TV-inspired concepts).
...
Per entry TF (default 15m,30m,1H):
  ...
  2) Modest grid per variant on train only (barrier netRT primary)
...
from keel.backtest.tv_compare import (  # noqa: E402
    TvStrategyConfig,
    barrier_metrics,
    modest_tv_grids,
    pack_row,
...
def _run_variant_on_tf(
    *,
    ...
    for i, cfg in enumerate(cells, start=1):
        summary = run_tv_config_on_window(
            train_series,
            cfg,
    ...
    ranked = sorted(
        train_rows,
        key=lambda r: (
            float((r.get("barrier") or {}).get("barrier_win_rate_net_rt") or -1.0),
            float((r.get("barrier") or {}).get("barrier_avg_net_rt_bps") or -1e9),
            int((r.get("train") or {}).get("full_gate_count") or 0),
        ),
        reverse=True,
    )
    chosen_row, selection_note = select_by_barrier(ranked)
    ...
    chosen_cfg = TvStrategyConfig(**chosen_row["config"])
    print(f"chosen: {chosen_cfg.label()}")
    print(
        f"train:  FG={chosen_row['train'].get('full_gate_count')} "
        f"bar_win={_fmt_rate((chosen_row.get('barrier') or {}).get('barrier_win_rate_net_rt'))} "
        f"bar_avg={_fmt_bps((chosen_row.get('barrier') or {}).get('barrier_avg_net_rt_bps'))} "
        f"fix_win={_fmt_rate(chosen_row['train'].get('win_rate_net_rt_5m'))}"
    )
    print(f"note:   {selection_note}")

    print("running frozen chosen on valid ...")
    valid_summary = run_tv_config_on_window(
        full_series,
        chosen_cfg,
    ...
    valid_bar = barrier_metrics(valid_summary)
    wr = valid_bar.get("barrier_win_rate_net_rt")
    avg = valid_bar.get("barrier_avg_net_rt_bps")
    n = int(valid_bar.get("barrier_n") or 0)
    ...
    if wr is None:
        verdict = f"{variant}/{spec.bar}: valid 无可用 barrier；不可宣称成功。E0 freeze。"
    elif passes:
        verdict = (
            f"{variant}/{spec.bar}: valid barrier win={_fmt_rate(wr)} "
            f"avg={_fmt_bps(avg)} n={n} ≥0.55 & avg≥0 & n≥20 — "
            f"仍须人工审阅；勿翻 .env。E0 freeze。"
        )
    else:
        verdict = (
            f"{variant}/{spec.bar}: valid barrier win={_fmt_rate(wr)} "
            f"avg={_fmt_bps(avg)} n={n} — 未过 0.55/avg≥0/n≥20。E0 freeze。"
        )
    print(
        f"valid:  FG={fg} bar_win={_fmt_rate(wr)} "
        f"bar_avg={_fmt_bps(avg)} n={n} "
        f"fix_win={_fmt_rate(valid_trainish.get('win_rate_net_rt_5m'))}"
    )
...
def _print_summary_table(results: list[dict[str, Any]]) -> None:
    print("\n=== F5 TV-inspired compare (barrier primary) ===")
    hdr = (
        f"{'TF':<4} {'variant':<13} {'cfg':<42} "
        f"{'tr_FG':>5} {'tr_bW':>8} {'tr_bAvg':>10} "
        f"{'va_FG':>5} {'va_bW':>8} {'va_bAvg':>10} {'va_n':>5} {'pass':>5}"
    )
    print(hdr)
    print("-" * len(hdr))
    for r in results:
        ...
        for variant in _VARIANTS:
            ...
            ch = vr.get("chosen") or {}
            tr = ch.get("train") or {}
            tb = ch.get("train_barrier") or {}
            va = ch.get("valid") or {}
            vb = ch.get("valid_barrier") or {}
            print(
                f"{r['entry_bar']:<4} {variant:<13} {str(ch.get('label',''))[:42]:<42} "
                f"{str(tr.get('full_gate_count')):>5} "
                f"{_fmt_rate(tb.get('barrier_win_rate_net_rt')):>8} "
                f"{_fmt_bps(tb.get('barrier_avg_net_rt_bps')):>10} "
                f"{str(va.get('full_gate_count')):>5} "
                f"{_fmt_rate(vb.get('barrier_win_rate_net_rt')):>8} "
                f"{_fmt_bps(vb.get('barrier_avg_net_rt_bps')):>10} "
                f"{str(vb.get('barrier_n')):>5} "
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="F5 TV-inspired strategy compare (public API; no keys)"
    )
    p.add_argument(
        "--inst-ids",
    ...
    print("F5 TV-inspired compare — public candles only; E0 freeze")
    print(
    ...
    for i, bar in enumerate(entry_bars):
        if i > 0:
            # OKX public rate-limit courtesy between TF fetches.
            time.sleep(12.0)
        try:
            results.append(
                _run_one_tf(
        ...
        except Exception as exc:  # noqa: BLE001 — report and continue other TFs
    ...
    if not any_pass:
        print(
            "\nHonest: no variant/TF cleared valid barrier ≥0.55 with "
            "avg_net≥0 and n≥20. Do not flip .env / clear kill."
        )
    else:
        print(
    ...
    payload = {
        "phase": "F5",
        "protocol": "tv_inspired_barrier_primary",
